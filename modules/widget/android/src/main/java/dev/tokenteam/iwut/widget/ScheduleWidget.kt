...
class ScheduleWidget : AppWidgetProvider() {
  ...
  companion object {
    fun updateWidget(
      ...
      val todayHint = if (upcomingToday.isEmpty()) "今天没有课了，" else "今天还有${upcomingToday.size}节课，"
      val tomorrowHint = if (tomorrowCourses.isEmpty()) "明天没有课了~" else "明天还有${tomorrowCourses.size}节课"
      views.setViewVisibility(R.id.tv_course_hint, View.VISIBLE)
      views.setTextViewText(R.id.tv_course_hint, todayHint + tomorrowHint)
